...
public sealed partial class SpreadsheetViewModel : ObservableObject
{
    ...
    public SpreadsheetViewModel(CalculatorSession session)
    {
    ...
    {
        string text = Input;
        CalcError? error = text.StartsWith('=')
            ? _sheet.SetFormula(Selected, text[1..])
            : _sheet.SetConstant(Selected, text);
    ...
    /// <summary>Calculates the sheet again (p. 104).</summary>
    [RelayCommand]
    ...
    /// <returns><see langword="true"/> when it was copied.</returns>
    public bool Copy(CellAddress from, CellAddress to)
    {
        CalcError? error = _sheet.CopyPaste(from, to);
        ErrorKey = error is { } fault ? "error." + fault.Kind : null;
        Refresh();
    ...
    /// <returns><see langword="true"/> when the range was filled.</returns>
    public bool Fill(string text, CellAddress start, CellAddress end)
    {
        ArgumentNullException.ThrowIfNull(text);
        CalcError? error = text.StartsWith('=') ? _sheet.Fill(text[1..], start, end) : _sheet.FillValue(text, start, end);
        ErrorKey = error is { } fault ? "error." + fault.Kind : null;
        Refresh();
        return error is null;
    }
